...
class AttentionSNELLParallel(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 low_rank_dim=None, tuning_model=None, bias=None, init_thres=0, norm_p=2, **kwargs):
...
class MlpSNELLParallel(nn.Module):
    """ MLP as used in Vision Transformer, MLP-Mixer and related networks
    """
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 low_rank_dim=None, tuning_model=None, bias=None, init_thres=0, norm_p=2, **kwargs):
...
class BlockSNELLParallel(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 low_rank_dim=None, tuning_model=None,
                 bias=None, init_thres=0, norm_p=2, **kwargs):

        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = AttentionSNELLParallel(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,
                                     low_rank_dim=low_rank_dim,
                                     tuning_model=tuning_model, bias=bias, init_thres=init_thres, norm_p=norm_p, **kwargs)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MlpSNELLParallel(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                              low_rank_dim=low_rank_dim,
                              tuning_model=tuning_model, bias=bias, init_thres=init_thres, norm_p=norm_p, **kwargs)
...
# Modified from https://github.com/ZhangYuanhan-AI/NOAH
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', freeze_backbone=False,
                 low_rank_dim=None, init_thres=0, **kwargs):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                low_rank_dim=low_rank_dim, init_thres=init_thres)
        ...
        if not any([freeze_backbone]):
            print('Fully fine-tuning!')
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        ...
        print('parameters:',total_para_nums,'adapter',adapter_para_nums,'LoRA',LoRA_para_nums,'prompt',vp_para_nums,'prefix',prefix_para_nums,'head',head_para_nums,'model',model_para_nums)
...
class VisionTransformerSNELL(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, weight_init='', freeze_backbone=False,
                 low_rank_dim=8, block=None, tuning_model=None, additional_bias=None,
                 init_thres=0, norm_p=2, **kwargs):
        super().__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.num_tokens = 2 if distilled else 1
        ...
        self.blocks = nn.Sequential(*[
            eval(block)(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                low_rank_dim=low_rank_dim, tuning_model=tuning_model, bias=additional_bias, init_thres=init_thres, norm_p=norm_p, **kwargs)
            for i in range(depth)])
        self.norm = norm_layer(embed_dim)
    ...
    @torch.jit.ignore()
    def load_pretrained(self, checkpoint_path, prefix=''):
        _load_weights_sep_qkv(self, checkpoint_path, prefix)
...
@torch.no_grad()
def _load_weights_sep_qkv(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    """ Load weights from .npz checkpoints for official Google Brain Flax implementation
    """
    import numpy as np
...
def _create_vision_transformer_sep_qkv(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    model = build_model_with_cfg(
        VisionTransformerSepQKV, variant, pretrained,
        default_cfg=default_cfg,
        representation_size=repr_size,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in default_cfg['url'],
...
def _create_vision_transformer_snell(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    model = build_model_with_cfg(
        VisionTransformerSNELL, variant, pretrained,
        default_cfg=default_cfg,
        representation_size=repr_size,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in default_cfg['url'],
...
@register_model
def vit_base_patch16_224_in21k_large_demo(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=1536, depth=8, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model
...
@register_model
def vit_base_patch16_224_in21k_sep_qkv(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer_sep_qkv('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model


@register_model
def vit_base_patch16_224_in21k_snell(pretrained=False, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    ...
    model = _create_vision_transformer_snell('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def vit_large_patch16_224_snell(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer_snell('vit_large_patch16_224_in21k', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
def vit_huge_patch14_224_snell(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_args = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16)
    model = _create_vision_transformer_snell('vit_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
